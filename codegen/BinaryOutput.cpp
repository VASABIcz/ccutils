...
Result<void> BinaryOutput::link() {
    ...
    for (const auto& [name, space] : spaces) {
        if (!labels.contains(name)) return FAIL("label \"{}\" not found", name);

        auto tag = labels[name];

        if (tag.size > space.size) return FAIL("symbol is dataSize of {} bytes, but space is dataSize of {} bytes", tag.size, space.size);
        TODO();
   /*     if (space.isRelative) {
