...
struct ControlFlowGraph {
    ...
    void
    discoverBackEdgesRec(BlockId node, std::map<BlockId, COLOR> &color, std::map<BlockId, std::set<BlockId>> &res) {
        color[node] = GREY;
    ...
    void filteredDFSRec(BlockId node, std::set<BlockId> &visited, std::vector<BlockId> &res,
                        const std::map<BlockId, std::set<BlockId>> &ignore) {
    ...
    pair<SSARegisterHandle, instructions::PhiFunction<CTX> *>
    makePhi(BaseBlock &current, span<pair<SSARegisterHandle, size_t>> regs) {
    ...
    void printRegisters() {
        for (const auto &reg: getRegistersConst()) {
            ...
            println("id: {} - name: {} - parent: {} - type: {} - size: {}", reg->getHandle().toString(), reg->name,
                    root == reg->getHandle() ? "_" : root.toString(), reg->typeString(), reg->sizeBytes());
        }
    }
    ...
    void registersToString(StringBuilder &sb) {
        for (const auto &reg: getRegistersConst()) {
            auto root = getRoot(reg->getHandle()).getHandle();
            sb.appendLine("id: {} - name: {} - parent: {} - type: {} - size: {}", reg->getHandle().toString(),
                          reg->name, root == reg->getHandle() ? "_" : root.toString(), reg->typeString(),
                          reg->sizeBytes());
        }
    }
    ...
    vector<BlockId> flattenBlocks() const {
        ...
        auto unresolvedIncomingEdges = [&](BlockId node) -> vector<BlockId> {
            return inverseTree[node] | views::filter([&](const auto &it) { return !resolved.contains(it); }) |
                   vi::toVec;
        };
        ...
        const auto removePriorityIfResolve = [&]() {
            ...
            priorityStack.erase(std::remove_if(priorityStack.begin(), priorityStack.end(),
                                               [&](const auto &it) { return isAllIncomingResolved(it); }),
                                priorityStack.end());
        };
        ...
        while (not toProcessStack.empty()) {
            ...
            if (not priorityStack.empty()) {
                res = indexOfLast(toProcessStack, [&](const auto &it) {
                    return canReach(it, priorityStack.back()) && isAllIncomingResolved(it);
                });
            }
            if (not res.has_value()) {
                res = indexOfLast(toProcessStack, [&](const auto &it) {
                    return !isAllIncomingResolved(it) && canReach(it, unresolvedIncomingEdges(it)[0]);
                });
            }
            if (not res.has_value()) {
            ...
            if (res.has_value()) {
                ...
                top = toProcessStack.back();
                toProcessStack.pop_back();
            }
    ...
    std::optional<BlockId> getLastBlockOfLoop(BlockId blockId, std::span<BlockId> blocks) const {
        ...
        auto ingressDist = collectVec(lookupIngress(blockId) | views::transform(
            [&](auto it) { return make_pair(it, (long) indexOf(blocks, it) - headerIndex); }));
        ...
        for (auto [ingId, ingOffset]: ingressDist) {
            if (ingOffset >= max.second)
                max = {ingId,
                       ingOffset}; // it can happend that this returns the inbound block that isnt actualy part of the loop
        }
    ...
    map<SSARegisterHandle, vector<bool>> liveRanges(vector<BlockId> blocks) const {
        ...
        auto addRange = [&](SSARegisterHandle handle, size_t s, size_t e, bool bypassStart = false) {
            ...
            auto start = (registerStarts.contains(handle) && !bypassStart) ? max(min(s, e), registerStarts[handle])
                                                                           : min(s, e);
    ...
    map<SSARegisterHandle, SSARegisterHandle> reachableDefinitions(const BaseBlock &block) const {
        ...
        while (currentBlock != nullptr) {
            ...
            for (const auto &regHandle: currentBlock->getInstructions() | views::transform(
                [&](const CopyPtr<IRInstruction<CTX>> &it) { return it->target; }) |
                                        views::filter([](const SSARegisterHandle &it) { return it.isValid(); }) |
                                        views::reverse) {
            ...
            currentBlock = currentBlock->previousId().has_value() ? &getBlockConst(*currentBlock->previousId())
                                                                  : nullptr;
        }
    ...
    void genPhiRec(BlockId reg, const std::set<SSARegisterHandle> &badBoys,
                   std::map<SSARegisterHandle, SSARegisterHandle> reachable, std::set<BlockId> &visited) {
    ...
    static std::map<size_t, std::set<size_t>>
    calcDom(std::set<size_t> nodes, std::function<std::vector<size_t>(size_t)> getPredecessors) {
        bool didChange = true;
    ...
    static std::map<size_t, std::set<size_t>>
    doStuff(size_t root, std::function<std::vector<size_t>(size_t)> succesors) {
        std::map<size_t, std::set<size_t>> res;
